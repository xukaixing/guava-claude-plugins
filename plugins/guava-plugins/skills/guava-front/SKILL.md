...
1. Read 配置（YAML + 表）；记下 YAML **`view`** 原文
2. [config-parser.md](config-parser.md) 推导清单 — **所有 Vue/helper/types 路径 = `src/views/<view>/...`**，禁止用 `.md` 路径或 `feature` 当目录
3. 展示清单时核对：例如 `view: sysMng/userMng2` → 必须出现 `src/views/sysMng/userMng2/`，不能是 `.../userMng/`
4. MCP：`get_page_recipe` + 相关 `gv-*` / `get_gv_component`
5. Write 每一个文件（覆盖策略见 \_shared.md）
6. Step 8

### Interactive Mode

确认：pageType → 基础字段（[config-parser.md](config-parser.md)）→ 查询（crud/tabs 必选，[search-conditions.md](search-conditions.md)，**校验必填**）→ 编辑字段（**校验必填**，同 search-conditions）→ CRUD 操作（禁止默认全开）→ tabs 列表（tabs 时）→ 按 pageType 选模板：
