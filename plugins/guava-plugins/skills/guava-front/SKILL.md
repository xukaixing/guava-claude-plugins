...
> [\_shared.md](_shared.md) · [conventions.md](conventions.md) · [code-review.md](code-review.md) · [../../context/front.md](../../context/front.md) · [MCP](../../mcp/guava-ui/README.md)
...
```
有 src/pages/**/*.md？
  是 → Read 配置 → config-parser + page-types → MCP page recipe → Write 全部 → Step 8 代码检查 → Step 9
...
确认：pageType → **是否 frontendOnly** → 基础字段（[config-parser.md](config-parser.md)）→ 查询（crud/tabs 必选，[search-conditions.md](search-conditions.md)，**校验必填**）→ 编辑字段（**校验必填**，同 search-conditions）→ CRUD 操作（禁止默认全开）→ tabs 列表（tabs 时）→ 按 pageType 选模板：
...
Write 完成后，**立即**按 [code-review.md](code-review.md) 对本轮文件做三检：

1. **代码规范**（conventions / Gv* / i18n / 文件头）
2. **代码安全**（XSS、密钥、校验、敏感数据）
3. **性能**（分页、watch、重复请求）

Critical 必须修复并再检；通过后输出三行简报（规范 / 安全 / 性能）。

### Step 9

清单 ✅ · i18n 已写 · Step 8 通过 · `lint-fix.sh` 无报错（见 \_shared.md）
