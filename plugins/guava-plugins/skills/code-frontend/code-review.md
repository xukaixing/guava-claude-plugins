...
| 类型 | 全局类型未 import；`FormInstance` / `TableInstance` 从 `element-plus` 导入 |
| JSDoc | 方法多行（`@todo:` / `@author:` / `@Date:`）；API 单行 `// xxx api` |
...
| 检查项 | 说明 |
| ------ | ---- |
| XSS | 无 `v-html` / `innerHTML` 渲染未消毒用户数据 |
| 敏感信息 | 无硬编码 token、密码、密钥、内网地址 |
| mock 数据 | `data.ts` 无真实 PII / 生产数据 |
| 写操作 | 删除 / 状态变更有确认；走 `crud.save` / `update` / `submit`，不绕过表单校验 |
| 入参 | 路由参数 / 查询入参与类型兜底 |
...
| 检查项 | 说明 |
| ------ | ---- |
| 分页 | 列表走 `crud.search`，无一次拉全量（mock 除外） |
| watch | 无多余 `watch`（能 `computed` 则不用） |
| 字典 | 不在循环内重复请求；复用已有 API |
| 大表单 | 无整表深拷贝；编辑回填用 `crud.setEditValue` |
| tabs 子表 | 按需查询，切换时不重复打相同接口 |
