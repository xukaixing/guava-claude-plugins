...
```
.Codex-plugin/marketplace.json      # marketplace + plugin manifest (version, owner)
plugins/guava-plugins/
  ...
  hooks/
    ...
    cursor-hooks.json.example        # non-Codex Agent equivalent
  mcp/
...
```bash
# Load plugin from local path into a Codex session
Codex --plugin-dir /path/to/guava-Codex-plugins/plugins/guava-plugins
...
After editing plugin files, run `/reload-plugins` inside Codex.
...
- Consumer settings live in the consumer's `.Codex/settings.json` (env + permissions), **not** here. This repo's `.Codex/settings.json` is for developing the plugin itself.
